package com.cocoamu.cloud.service.async;

import com.alibaba.fastjson.JSON;

import com.cocoamu.cloud.constant.GoodsConstant;
import com.cocoamu.cloud.dao.GoodsDao;
import com.cocoamu.cloud.entity.GoodsEntity;
import com.cocoamu.cloud.goods.GoodsInfo;
...
public class AsyncServiceImpl implements IAsyncService {
    ...
    private final GoodsDao ecommerceGoodsDao;
    private final StringRedisTemplate redisTemplate;

    public AsyncServiceImpl(GoodsDao ecommerceGoodsDao,
    ...
    public void asyncImportGoods(List<GoodsInfo> goodsInfos, String taskId) {
        ...
        List<GoodsEntity> ecommerceGoods = filteredGoodsInfo.stream()
                .map(GoodsEntity::to)
                .collect(Collectors.toList());
        List<GoodsEntity> targetGoods = new ArrayList<>(ecommerceGoods.size());
        ...
        // 商品信息入库
        List<GoodsEntity> savedGoods = IterableUtils.toList(
     ...
     * */
    private void saveNewGoodsInfoToRedis(List<GoodsEntity> savedGoods) {

        // 由于 Redis 是内存存储, 只存储简单商品信息
        List<SimpleGoodsInfo> simpleGoodsInfos = savedGoods.stream()
                .map(GoodsEntity::toSimple)
                .collect(Collectors.toList());
